...
@section('content')
<div class="mb-6">
    <a href="{{ route('mahasiswa.courses.forum.index', $course->id) }}" class="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-cyan-600 transition mb-4">
            ...
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    ...
                    <div id="reply-form-{{ $reply->id }}" class="hidden mb-4">
                        <form action="{{ route('mahasiswa.courses.forum.reply', [$course->id, $discussion->id]) }}" method="POST">
                            @csrf
...
<div class="mt-8 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
    <h4 class="font-bold text-slate-800 dark:text-white mb-4">Tulis Balasan</h4>
    <form action="{{ route('mahasiswa.courses.forum.reply', [$course->id, $discussion->id]) }}" method="POST">
        @csrf
        <textarea name="body" required rows="4" placeholder="Tulis pendapat Anda di sini..." class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-cyan-500 dark:text-white resize-none mb-3"></textarea>
