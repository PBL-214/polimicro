...
<div class="mb-6">
    <select id="filter" class="px-4 py-3 rounded-xl border border-gray-200 bg-white text-sm" onchange="window.location='{{ route('mahasiswa.materials') }}?matkul='+this.value">
    ...
    @forelse($filtered as $m)
        ...
        <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            <div class="p-5 border-b border-gray-50 flex items-center gap-3">
                ...
                <div><h3 class="font-bold text-gray-900">{{ $m->nama_makul }}</h3><p class="text-xs text-gray-400">{{ $m->materi->count() }} materi tersedia</p></div>
            </div>
                ...
                <div class="p-5 hover:bg-cyan-50/50 transition cursor-pointer" onclick="this.querySelector('.detail').classList.toggle('hidden')">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div><p class="font-semibold text-sm">{{ $mat->nama_materi }}</p><p class="text-xs text-gray-400">{{ $mat->created_at->translatedFormat('j F Y') }}</p></div>
                        </div>
                    ...
                    <div class="detail hidden mt-4 p-4 bg-gray-50 rounded-xl text-sm text-gray-600">
                        <p>{{ $mat->deskripsi_materi }}</p>
                        <div class="mt-3 flex items-center gap-2">
                            @if($mat->file_materi)
                                <a href="{{ asset('storage/' . $mat->file_materi) }}" target="_blank" class="px-4 py-2 bg-[#0E7490] text-white rounded-lg text-xs font-medium hover:bg-[#155E75] transition inline-flex items-center gap-2">
                                    <i class="fas fa-file-download"></i>
                                    {{ basename($mat->file_materi) }}
                                </a>
                            @else
