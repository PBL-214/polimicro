@extends('layouts.dashboard', ['activePage' => 'grades'])
@section('title', 'Nilai - Polimicro')
@section('content')
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-5 py-4 font-medium">
                            <div class="flex items-center gap-2">
                                {{ $s->tugas->nama_tugas ?? '-' }}
                                <a href="{{ asset('storage/' . $s->file_dikumpul) }}" target="_blank" class="text-cyan-600 hover:text-cyan-700" title="Lihat file saya"><i class="fas fa-file-pdf"></i></a>
                            </div>
                        </td>
                        <td class="px-5 py-4 text-gray-500">{{ $s->tugas->makul->nama_makul ?? '-' }}</td>
                        <td class="px-5 py-4 text-gray-400">{{ $s->waktu_kumpul?->format('d/m/Y') ?? '-' }}</td>
                        <td class="px-5 py-4 text-center font-bold">{{ $s->nilai !== null ? $s->nilai : '-' }}</td>
                        <td class="px-5 py-4 text-center">@if($grade)<span class="px-3 py-1 rounded-full text-xs font-bold {{ $grade[1] }}">{{ $grade[0] }}</span>@else - @endif</td>
                        <td class="px-5 py-4 text-gray-500 text-xs max-w-[200px] truncate">{{ $s->feedback ?? '-' }}</td>
                    </tr>
                @empty
