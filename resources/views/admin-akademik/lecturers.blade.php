...
<div class="flex items-center justify-between mb-6">
    ...
    <div class="flex gap-3">
        <a href="{{ route('admin-akademik.lecturers.export') }}" class="px-5 py-3 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-xl font-bold text-sm hover:bg-emerald-100 transition inline-flex items-center gap-2">
    ...
    <div class="overflow-x-auto"><table class="w-full text-sm">
        ...
        @foreach($dosenList as $d)
            ...
            <tr class="hover:bg-slate-50 group/row transition">
                <td class="px-5 py-4"><div class="flex items-center gap-3"><div class="w-10 h-10 rounded-full bg-cyan-600 flex items-center justify-center text-white text-xs font-bold transition-transform group-hover/row:scale-110 shadow-lg shadow-cyan-600/20">{{ $d->getInitials() }}</div><div><p class="font-medium text-slate-800">{{ $d->name }}</p><p class="text-xs text-slate-400">{{ $d->phone ?? '' }}</p></div></div></td>
                <td class="px-5 py-4 text-slate-500 font-mono text-xs">{{ $d->nip ?? '-' }}</td>
                ...
                <td class="px-5 py-4 text-slate-500 text-xs">{{ $d->email }}</td>
                <td class="px-5 py-4 text-center"><span class="px-3 py-1 rounded-full text-xs font-bold bg-cyan-100 text-cyan-700 border border-cyan-100">{{ $mc }}</span></td>
                <td class="px-5 py-4 text-center">
                    <div class="row-actions flex gap-2 justify-center">
                        <button onclick="editDosen({{ $d->id }},'{{ addslashes($d->name) }}','{{ $d->email }}','{{ $d->nip }}','{{ $d->phone }}','{{ addslashes($d->homebase ?? '') }}','{{ addslashes($d->address ?? '') }}')" class="px-3 py-2 bg-cyan-50 text-cyan-600 rounded-lg text-xs hover:bg-cyan-100 transition border border-cyan-100"><i class="fas fa-edit"></i></button>
                        <form method="POST" action="{{ route('admin-akademik.lecturers.destroy', $d) }}">
                            @csrf @method('DELETE')
