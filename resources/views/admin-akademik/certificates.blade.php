...
<div class="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
    <div class="overflow-x-auto"><table class="w-full text-sm">
        ...
        @forelse($certs as $c)
            <tr class="hover:bg-slate-50/80 transition cert-row group" data-info="{{ strtolower($c->nomor_sertifikat . ' ' . ($c->mahasiswa->name ?? '')) }}">
                <td class="px-5 py-4 font-mono text-xs font-bold text-cyan-600">{{ $c->nomor_sertifikat }}</td>
                <td class="px-5 py-4"><div class="flex items-center gap-3"><div class="w-8 h-8 rounded-full bg-cyan-100 flex items-center justify-center text-[10px] font-bold text-cyan-700 transition-transform group-hover:scale-110 shadow-sm border border-white">{{ $c->mahasiswa ? $c->mahasiswa->getInitials() : '?' }}</div><span class="font-bold text-slate-800">{{ $c->mahasiswa->name ?? '-' }}</span></div></td>
                <td class="px-5 py-4 text-slate-500 text-xs font-medium">{{ $c->prodi->nama_prodi ?? '-' }}</td>
                <td class="px-5 py-4">
                    <p class="text-slate-400 text-[10px] mb-1 font-bold">{{ $c->tanggal_terbit->format('d M Y') }}</p>
                    @if($c->file_sertifikat)
                        <a href="{{ asset('storage/' . $c->file_sertifikat) }}" target="_blank" class="text-cyan-600 hover:text-cyan-700 font-bold text-[10px] inline-flex items-center gap-1 transition-colors"><i class="fas fa-external-link-alt"></i> UNDUH PDF</a>
                    @else
                        <span class="text-xs text-slate-300">-</span>
                    @endif
                </td>
                <td class="px-5 py-4 text-center"><span class="px-2.5 py-0.5 rounded-lg text-[10px] font-bold uppercase bg-cyan-50 text-cyan-700 border border-cyan-100">{{ $c->status }}</span></td>
                <td class="px-5 py-4 text-center">
                    <div class="row-actions flex gap-2 justify-center">
                        <form method="POST" action="{{ route('admin-akademik.certificates.destroy', $c) }}">
                            @csrf @method('DELETE')
...
<div id="cert-modal" class="fixed inset-0 z-50 hidden items-center justify-center" style="background:rgba(15,23,42,0.6); backdrop-filter: blur(4px);">
    <div class="bg-white rounded-[2.5rem] shadow-2xl p-10 max-w-lg w-full mx-4 fade-in">
        ...
        <form method="POST" action="{{ route('admin-akademik.certificates.store') }}" enctype="multipart/form-data" class="space-y-5" onsubmit="this.querySelector('[data-submit]').disabled=true;this.querySelector('[data-submit]').innerHTML='<i class=\'fas fa-spinner fa-spin text-sm\'></i> Menerbitkan...'">@csrf
            <div><label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Mahasiswa <span class="text-red-400">*</span></label><select name="mahasiswa_id" required class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-cyan-500/10 focus:border-cyan-500 outline-none transition">@foreach($mahasiswaList as $m)<option value="{{ $m->id }}">{{ $m->name }} ({{ $m->nim }})</option>@endforeach</select></div>
            <div><label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Program Studi <span class="text-red-400">*</span></label><select name="prodi_id" required class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-cyan-500/10 focus:border-cyan-500 outline-none transition">@foreach($prodiList as $p)<option value="{{ $p->id }}">{{ $p->nama_prodi }}</option>@endforeach</select></div>
            <div class="grid grid-cols-2 gap-4">
                <div><label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">No. Sertifikat <span class="text-red-400">*</span></label><input type="text" name="nomor_sertifikat" value="CERT-PM-{{ date('Y') }}-{{ str_pad($certs->count()+1, 3, '0', STR_PAD_LEFT) }}" required class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-cyan-500/10 focus:border-cyan-500 outline-none transition"></div>
                <div><label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Tgl Terbit <span class="text-red-400">*</span></label><input type="date" name="tanggal_terbit" value="{{ date('Y-m-d') }}" required class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-cyan-500/10 focus:border-cyan-500 outline-none transition"></div>
            </div>
            <div>
