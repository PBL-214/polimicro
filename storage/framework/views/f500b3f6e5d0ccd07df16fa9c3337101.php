...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'Polimicro'); ?></title>
...
<body class="bg-slate-50 text-slate-700">
    ...
    <aside id="sidebar" class="fixed left-0 top-0 h-full w-64 z-40 flex flex-col transition-transform duration-300 lg:translate-x-0 -translate-x-full shadow-2xl lg:shadow-none bg-slate-800">
        <div class="p-6 border-b border-white/10">
            <a href="<?php echo e(route('programs')); ?>" class="flex items-center gap-3">
        ...
        <div class="p-4 mx-4 mt-4 rounded-2xl bg-white/5 border border-white/5">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-full bg-cyan-500/20 border border-cyan-500/30 flex items-center justify-center font-bold text-sm text-cyan-300 shadow-inner"><?php echo e($initials); ?></div>
                <div class="overflow-hidden">
                    <p class="font-bold text-sm truncate text-white tracking-wide"><?php echo e($user->name); ?></p>
                    <p class="text-[10px] font-bold uppercase tracking-widest text-cyan-400/80"><?php echo e($user->getRoleLabel()); ?></p>
                </div>
            </div>
        </div>
        <nav class="flex-1 p-4 space-y-6 overflow-y-auto custom-scrollbar">
            <?php $__currentLoopData = $itemsByGroup; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $group => $items): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="space-y-1">
                    <p class="text-[10px] text-white/30 font-bold uppercase tracking-[0.2em] px-4 mb-2"><?php echo e($group); ?></p>
                    <?php $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <a href="<?php echo e(route($item['route'])); ?>" onclick="startProgress()" class="flex items-center gap-3 px-4 py-2.5 text-sm font-medium rounded-xl transition-all duration-200 <?php echo e($activePage === $item['id'] ? 'bg-cyan-500 text-white shadow-lg shadow-cyan-500/20' : 'text-slate-400 hover:text-white hover:bg-white/[0.05]'); ?>">
                            <i class="<?php echo e($item['icon']); ?> w-5 text-center text-xs opacity-80"></i><?php echo e($item['label']); ?>
        ...
        <div class="p-4 border-t border-white/10 mb-20 lg:mb-0">
            <a href="<?php echo e(route('programs')); ?>" class="flex items-center gap-3 px-4 py-3 text-sm text-white/60 hover:text-white rounded-xl hover:bg-white/[0.08] transition"><i class="fas fa-compass w-5 text-center"></i>Jelajahi Program</a>
    ...
    <div id="main-content" class="lg:ml-64 transition-all">
        ...
        <header class="sticky top-0 z-30 border-b" style="background:rgba(248,250,252,0.9);backdrop-filter:blur(12px);border-color:#e2e8f0;">
            <div class="flex items-center justify-between px-6 py-4">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="relative group">
                        <button class="relative text-gray-400 hover:text-gray-900 transition">
                            ...
                            <?php if($unreadCount > 0): ?>
                                <span class="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white text-[9px] font-bold rounded-full border-2 border-white flex items-center justify-center"><?php echo e($unreadCount); ?></span>
                            <?php endif; ?>
                        </button>
                        <div class="absolute right-0 mt-3 w-80 glass rounded-2xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 overflow-hidden">
                            ...
                            <div class="max-h-[350px] overflow-y-auto">
                                <?php $__empty_1 = true; $__currentLoopData = $notifications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $notif): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    <a href="<?php echo e(route('notifications.click', $notif->id)); ?>" class="p-4 hover:bg-cyan-50/50 transition border-b border-gray-50 flex gap-3 block">
                                        <div class="w-8 h-8 rounded-full <?php echo e($notif->data['color'] ?? 'bg-cyan-100'); ?> flex items-center justify-center <?php echo e($notif->data['text_color'] ?? 'text-cyan-600'); ?> flex-shrink-0">
                                            <i class="<?php echo e($notif->data['icon'] ?? 'fas fa-bell'); ?> text-xs"></i>
                                        </div>
                                        <div>
                                            <p class="text-xs font-semibold text-slate-800"><?php echo e($notif->data['title'] ?? 'Notifikasi Baru'); ?></p>
                                            <p class="text-[10px] text-gray-400 leading-tight mt-0.5"><?php echo e($notif->data['message'] ?? ''); ?></p>
                                            <p class="text-[9px] text-cyan-500 mt-1"><?php echo e($notif->created_at->diffForHumans()); ?></p>
                                        </div>
                                    </a>
                    ...
                    <div class="w-9 h-9 rounded-full flex items-center justify-center text-white text-sm font-bold shadow-lg shadow-cyan-500/20" style="background:linear-gradient(135deg,#1e293b,#06b6d4)"><?php echo e($initials); ?></div>
                </div>
            </div>
        </header>
        ...
        <?php if(session('success')): ?>
            <div id="toast-success" class="fixed top-6 right-6 z-[9999] px-6 py-3 rounded-xl text-white bg-cyan-500 shadow-2xl transform transition-all duration-500">
                <div class="flex items-center gap-3"><span><?php echo e(session('success')); ?></span></div>
            </div>
        ...
        <?php if(session('warning')): ?>
            <div id="toast-warning" class="fixed top-6 right-6 z-[9999] px-6 py-3 rounded-xl text-white bg-yellow-500 shadow-2xl transform transition-all duration-500">
                <div class="flex items-center gap-3"><span><?php echo e(session('warning')); ?></span></div>
            </div>
        ...
        <?php if($errors->any()): ?>
            <div id="toast-error" class="fixed top-6 right-6 z-[9999] px-6 py-3 rounded-xl text-white bg-red-500 shadow-2xl transform transition-all duration-500">
                <div class="flex items-center gap-3"><i class="fas fa-exclamation-circle"></i><span><?php echo e($errors->first()); ?></span></div>
            </div>
    ...
    <div class="bottom-nav lg:hidden border-t border-slate-100 bg-white/80 backdrop-blur-lg">
        <?php $__currentLoopData = $currentBottomNav; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <a href="<?php echo e(route($item['route'])); ?>" class="bottom-nav-item <?php echo e($activePage === $item['id'] ? 'text-cyan-600' : 'text-slate-400'); ?>">
                <i class="<?php echo e($item['icon']); ?>"></i>
                <span class="text-[10px] font-bold uppercase tracking-tighter"><?php echo e($item['label']); ?></span>
            </a>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
...
<?php /**PATH C:\laragon\www\polimicro\resources\views/layouts/dashboard.blade.php ENDPATH**/ ?>