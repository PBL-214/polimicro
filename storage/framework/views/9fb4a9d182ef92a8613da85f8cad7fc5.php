    ...
    <nav class="sticky top-0 z-50 px-8 py-5 flex items-center justify-between border-b border-white/20 glass-card">
        ...
        <div class="flex items-center gap-6">
            <?php if(auth()->guard()->check()): ?>
                <a href="<?php echo e(route(auth()->user()->getDashboardRoute())); ?>" class="flex items-center gap-3 px-4 py-2 rounded-2xl bg-white/50 border border-white hover:bg-white transition shadow-sm">
                    <div class="w-8 h-8 rounded-full bg-cyan-600 text-white flex items-center justify-center font-bold text-xs shadow-sm"><?php echo e(auth()->user()->getInitials()); ?></div>
                    <span class="text-sm font-semibold text-slate-700 hidden md:inline"><?php echo e(auth()->user()->name); ?></span>
                </a>
            <?php else: ?>
                <a href="<?php echo e(route('login')); ?>" class="text-sm font-semibold text-slate-600 hover:text-cyan-600 transition">Masuk</a>
                <a href="<?php echo e(route('register')); ?>" class="px-6 py-3 bg-cyan-600 text-white rounded-2xl font-bold text-sm shadow-xl shadow-cyan-600/20 hover:bg-cyan-700 transition active:scale-95">Mulai Belajar</a>
            <?php endif; ?>
        </div>
    </nav>
            ...
            <div class="swiper-wrapper">
                ...
                <div class="swiper-slide h-auto">
                    <div class="program-card glass-card rounded-[2.5rem] p-8 transition-all duration-500 flex flex-col h-full relative group">
                        ...
                        <div class="flex-1">
                            <h3 class="text-2xl font-bold font-serif text-slate-900 mb-3 group-hover:text-cyan-600 transition-colors"><?php echo e($p->nama_prodi); ?></h3>
                            <p class="text-slate-500 text-sm leading-relaxed mb-8 line-clamp-3">
                                <?php echo e($p->deskripsi ?: 'Pelajari keahlian mendalam dalam bidang ini dengan kurikulum yang terstruktur dan dimentoring oleh para profesional berpengalaman.'); ?>

                            </p>

                            <div class="flex items-center gap-6 mb-8 py-4 border-y border-slate-100">
                                <div class="flex flex-col">
                                    <span class="text-slate-900 font-bold text-lg"><?php echo e($mkCount); ?></span>
                                    <span class="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">Mata Kuliah</span>
                                </div>
                                <div class="w-px h-8 bg-slate-100"></div>
                                <div class="flex flex-col">
                                    <span class="text-slate-900 font-bold text-lg"><?php echo e($enrollCount); ?></span>
                        ...
                        <?php if(auth()->guard()->check()): ?>
                            <?php if(auth()->user()->isMahasiswa()): ?>
                                ...
                                <?php else: ?>
                                    <form method="POST" action="<?php echo e(route('programs.enroll')); ?>">
                                        <?php echo csrf_field(); ?>
                                        <input type="hidden" name="prodi_id" value="<?php echo e($p->id); ?>">
                            ...
                            <?php endif; ?>
                        <?php else: ?>
                            <a href="<?php echo e(route('register')); ?>" class="w-full py-4 bg-cyan-600 text-white text-center rounded-2xl font-bold text-sm shadow-xl shadow-cyan-600/10 hover:bg-cyan-700 transition-all duration-300">
...
<?php /**PATH C:\laragon\www\polimicro\resources\views/programs.blade.php ENDPATH**/ ?>