...
<body class="min-h-screen flex bg-slate-50 text-slate-800">
    ...
    <div class="flex-1 flex items-center justify-center p-8 bg-slate-50">
        <div class="w-full max-w-md fade-in">
            ...
            <?php if($errors->any()): ?>
                <div class="mb-4 p-4 bg-red-50 border border-red-200 rounded-xl text-red-600 text-sm"><?php echo e($errors->first()); ?></div>
            <?php endif; ?>
            <form method="POST" action="<?php echo e(route('register')); ?>" class="space-y-4">
                ...
                <div><label class="block text-sm font-medium text-slate-700 mb-1.5">Nama Lengkap</label>
                    ...
                    <input type="text" name="nama" value="<?php echo e(old('nama')); ?>" required placeholder="Masukkan nama lengkap" class="w-full pl-11 pr-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 bg-white transition"></div>
                </div>
                <div><label class="block text-sm font-medium text-slate-700 mb-1.5">Email</label>
                    ...
                    <input type="email" name="email" value="<?php echo e(old('email')); ?>" required placeholder="nama@email.com" class="w-full pl-11 pr-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 bg-white transition"></div>
                </div>
                <div><label class="block text-sm font-medium text-slate-700 mb-1.5">No. Telepon</label>
                    ...
                    <input type="tel" name="phone" value="<?php echo e(old('phone')); ?>" placeholder="08xxxxxxxxxx" class="w-full pl-11 pr-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 bg-white transition"></div>
                </div>
                <div><label class="block text-sm font-medium text-slate-700 mb-1.5">Program Studi</label>
                    ...
                    <select name="prodi_id" required class="w-full pl-11 pr-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 bg-white transition appearance-none cursor-pointer">
                        ...
                        <?php $__currentLoopData = $programs ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $program): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($program->id); ?>"><?php echo e($program->nama_prodi); ?></option>
            ...
            <p class="text-center text-slate-500 mt-6">Sudah punya akun? <a href="<?php echo e(route('login')); ?>" class="text-cyan-600 font-semibold hover:text-cyan-700">Masuk</a></p>
        </div>
    </div>
</body>
</html>
<?php /**PATH C:\laragon\www\polimicro\resources\views/auth/register.blade.php ENDPATH**/ ?>