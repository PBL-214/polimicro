<?php $__env->startSection('title', 'Dashboard Mahasiswa - Polimicro'); ?>
<?php $__env->startSection('content'); ?>
<div class="mb-8">
    <h1 class="text-2xl font-bold text-gray-900">Selamat Datang, <?php echo e(explode(' ', $user->name)[0]); ?>! 👋</h1>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    <div class="bg-white rounded-2xl p-5 border border-gray-100 card-hover">
        ...
        <p class="text-2xl font-bold text-gray-900"><?php echo e($enrolled->count()); ?></p><p class="text-xs text-gray-400 mt-1">Mata Kuliah</p>
    </div>
    <div class="bg-white rounded-2xl p-5 border border-gray-100 card-hover">
        ...
        <p class="text-2xl font-bold text-gray-900"><?php echo e($submissions->count()); ?></p><p class="text-xs text-gray-400 mt-1">Tugas Dikumpul</p>
    </div>
    <div class="bg-white rounded-2xl p-5 border border-gray-100 card-hover">
        ...
        <p class="text-2xl font-bold text-gray-900"><?php echo e($avgGrade); ?></p><p class="text-xs text-gray-400 mt-1">Rata-rata Nilai</p>
    </div>
    <div class="bg-white rounded-2xl p-5 border border-gray-100 card-hover">
        ...
        <p class="text-2xl font-bold text-gray-900"><?php echo e($certs->count()); ?></p><p class="text-xs text-gray-400 mt-1">Sertifikat</p>
    </div>
</div>
        ...
        <div class="grid grid-cols-3 sm:grid-cols-5 gap-6">
            ...
            <?php $__currentLoopData = $badges; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $badge): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="badge-container text-center group/badge relative cursor-help">
                    <div class="badge-card relative w-16 h-16 mx-auto rounded-2xl flex items-center justify-center text-2xl shadow-lg transition-all duration-300 <?php echo e($badge['status'] === 'unlocked' ? 'bg-white border-2 border-cyan-100 group-hover/badge:scale-110 group-hover/badge:rotate-3 group-hover/badge:shadow-cyan-600/20' : 'bg-gray-100 grayscale opacity-40'); ?>">
                        <?php echo e($badge['icon']); ?>

                        <?php if($badge['status'] === 'unlocked'): ?> <div class="badge-glow"></div> <?php endif; ?>
                    </div>
                    <p class="text-[10px] font-bold mt-2 uppercase tracking-wider <?php echo e($badge['status'] === 'unlocked' ? 'text-cyan-600' : 'text-gray-300'); ?>"><?php echo e($badge['label']); ?></p>
                    ...
                    <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-32 p-2 bg-slate-900 text-white text-[10px] rounded-lg opacity-0 invisible group-hover/badge:opacity-100 group-hover/badge:visible transition-all duration-300 z-20 pointer-events-none">
                        <p class="font-bold mb-1"><?php echo e($badge['status'] === 'unlocked' ? 'Unlocked! 🎉' : 'Locked 🔒'); ?></p>
                        <p class="opacity-70"><?php echo e($badge['hint']); ?></p>
    ...
    <div class="bg-white rounded-3xl border border-gray-100 p-8 flex flex-col items-center justify-center text-center shadow-sm">
        <div class="relative w-32 h-32 mb-4">
            <svg class="w-full h-full" viewBox="0 0 36 36">
                ...
                <path class="text-cyan-500" stroke-dasharray="<?php echo e($overallProgress); ?>, 100" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" />
            </svg>
            <div class="absolute inset-0 flex flex-col items-center justify-center">
                <span class="text-2xl font-bold text-gray-900"><?php echo e($overallProgress); ?>%</span>
...
<div class="grid lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
        <div class="flex items-center justify-between mb-8">
            ...
            <a href="<?php echo e(route('mahasiswa.courses')); ?>" class="px-4 py-2 bg-slate-50 text-slate-600 rounded-xl text-xs font-bold hover:bg-slate-100 transition border border-slate-200">Lihat Katalog</a>
        </div>
        <div class="relative space-y-8 pl-4">
            <div class="roadmap-connector"></div>
            <?php $__empty_1 = true; $__currentLoopData = $enrolled->take(4); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="roadmap-dot flex items-center gap-6 group">
                    ...
                    <a href="<?php echo e(route('mahasiswa.materials', ['matkul' => $m->id])); ?>" class="flex-1 p-5 rounded-2xl bg-slate-50 border border-transparent hover:border-cyan-200 hover:bg-white transition-all duration-300 group-hover:shadow-xl group-hover:shadow-black/5">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="font-bold text-gray-900 group-hover:text-cyan-600 transition"><?php echo e($m->nama_makul); ?></p>
                                <p class="text-xs text-gray-400 mt-1"><?php echo e($m->prodi->nama_prodi ?? ''); ?> • <?php echo e($m->dosen->name ?? ''); ?></p>
                            </div>
                            <div class="text-right">
                                <span class="text-xs font-bold text-cyan-600 bg-cyan-50 px-2 py-1 rounded-lg border border-cyan-100"><?php echo e($m->progress); ?>% Selesai</span>
                            </div>
                        </div>
                    </a>
                </div>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.empty-state','data' => ['icon' => 'fas fa-route','title' => 'Mulai Perjalanan Anda','description' => 'Anda belum mengambil mata kuliah apapun. Jelajahi katalog program kami.','actionText' => 'Jelajahi Program','actionUrl' => route('programs')]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['icon' => 'fas fa-route','title' => 'Mulai Perjalanan Anda','description' => 'Anda belum mengambil mata kuliah apapun. Jelajahi katalog program kami.','actionText' => 'Jelajahi Program','actionUrl' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(route('programs'))]); ?>
...
<?php endif; ?>
    ...
    <div class="bg-white rounded-3xl border border-gray-100 p-8">
        ...
        <div class="space-y-4">
            <?php $__empty_1 = true; $__currentLoopData = $upcoming->take(5); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="p-4 rounded-2xl bg-white border border-gray-100 hover:border-amber-200 hover:shadow-lg hover:shadow-amber-500/5 transition duration-300">
                    <p class="font-bold text-sm text-gray-800"><?php echo e($t->nama_tugas); ?></p>
                    <p class="text-[11px] text-gray-400 mt-1 uppercase tracking-wider font-semibold"><?php echo e($t->matkul_nama); ?></p>
                    <div class="flex items-center justify-between mt-3">
                        <span class="text-[10px] px-2 py-1 bg-amber-50 text-amber-600 rounded-lg font-bold"><i class="fas fa-calendar-alt mr-1"></i><?php echo e($t->tanggal_akhir_deadline?->translatedFormat('d M') ?? '-'); ?></span>
            ...
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                <?php if (isset($component)) { $__componentOriginal074a021b9d42f490272b5eefda63257c = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal074a021b9d42f490272b5eefda63257c = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.empty-state','data' => ['icon' => 'fas fa-calendar-check','title' => 'Semua Beres!','description' => 'Tidak ada tugas mendatang untuk saat ini.','class' => 'py-10!']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
<?php $component->withName('empty-state'); ?>
<?php if ($component->shouldRender()): ?>
<?php $__env->startComponent($component->resolveView(), $component->data()); ?>
<?php if (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag): ?>
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['icon' => 'fas fa-calendar-check','title' => 'Semua Beres!','description' => 'Tidak ada tugas mendatang untuk saat ini.','class' => 'py-10!']); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
<?php if (isset($__attributesOriginal074a021b9d42f490272b5eefda63257c)): ?>
<?php $attributes = $__attributesOriginal074a021b9d42f490272b5eefda63257c; ?>
...
<?php echo $__env->make('layouts.dashboard', ['activePage' => 'dashboard'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\polimicro\resources\views/mahasiswa/dashboard.blade.php ENDPATH**/ ?>